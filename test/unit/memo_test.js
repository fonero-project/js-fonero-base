...
describe("Memo.constructor()", function() {
  it("throws error when type is invalid", function() {
    expect(() => new FoneroBase.Memo("test")).to.throw(/Invalid memo type/);
  });
});

describe("Memo.none()", function() {
  it("converts to/from xdr object", function() {
    let memo = FoneroBase.Memo.none().toXDRObject();
    expect(memo.value()).to.be.undefined;

    let baseMemo = FoneroBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(FoneroBase.MemoNone);
...
describe("Memo.text()", function() {
  ...
  it("returns a value for a correct argument", function() {
    expect(() => FoneroBase.Memo.text("test")).to.not.throw();
    let memoUtf8 = FoneroBase.Memo.text("三代之時")
  ...
  it("returns a value for a correct argument (utf8)", function() {
    let memoText = FoneroBase.Memo.text([0xd1]).toXDRObject().toXDR();
    ...
    memoText = FoneroBase.Memo.text(Buffer.from([0xd1])).toXDRObject().toXDR();
    expect(memoText.equals(expected)).to.be.true;
  });

  it("converts to/from xdr object", function() {
    let memo = FoneroBase.Memo.text("test").toXDRObject();
    expect(memo.arm()).to.equal('text');
    expect(memo.text()).to.equal('test');
    expect(memo.value()).to.equal('test');

    let baseMemo = FoneroBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(FoneroBase.MemoText);
    expect(baseMemo.value).to.be.equal('test');
  });

  it("converts to/from xdr object (array)", function() {
    let memo = FoneroBase.Memo.text([0xd1]).toXDRObject();
    expect(memo.arm()).to.equal('text');
  ...
  it("converts to/from xdr object (buffer)", function() {
    ...
    let memo = FoneroBase.Memo.text(buf).toXDRObject();
    expect(memo.arm()).to.equal('text');
  ...
  it("throws an error when invalid argument was passed", function() {
    expect(() => FoneroBase.Memo.text()).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => FoneroBase.Memo.text({})).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => FoneroBase.Memo.text(10)).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => FoneroBase.Memo.text(Infinity)).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => FoneroBase.Memo.text(NaN)).to.throw(/Expects string, array or buffer, max 28 bytes/);
  });

  it("throws an error when string is longer than 28 bytes", function() {
    expect(() => FoneroBase.Memo.text("12345678901234567890123456789")).to.throw(/Expects string, array or buffer, max 28 bytes/);
    expect(() => FoneroBase.Memo.text("三代之時三代之時三代之時")).to.throw(/Expects string, array or buffer, max 28 bytes/);
  });
...
describe("Memo.id()", function() {
  it("returns a value for a correct argument", function() {
    expect(() => FoneroBase.Memo.id("1000")).to.not.throw();
    expect(() => FoneroBase.Memo.id("0")).to.not.throw();
  });

  it("converts to/from xdr object", function() {
    let memo = FoneroBase.Memo.id("1000").toXDRObject();
    expect(memo.arm()).to.equal('id');
    expect(memo.id().toString()).to.equal('1000');

    let baseMemo = FoneroBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(FoneroBase.MemoID);
    expect(baseMemo.value).to.be.equal('1000');
  });

  it("throws an error when invalid argument was passed", function() {
    expect(() => FoneroBase.Memo.id()).to.throw(/Expects a int64/);
    expect(() => FoneroBase.Memo.id({})).to.throw(/Expects a int64/);
    expect(() => FoneroBase.Memo.id(Infinity)).to.throw(/Expects a int64/);
    expect(() => FoneroBase.Memo.id(NaN)).to.throw(/Expects a int64/);
    expect(() => FoneroBase.Memo.id("test")).to.throw(/Expects a int64/);
  });
});
...
describe("Memo.hash() & Memo.return()", function() {
  ...
  it("return converts to/from xdr object", function() {
    ...
    // Testing string hash
    let memo = FoneroBase.Memo.return(buffer.toString("hex")).toXDRObject();
    ...
    let baseMemo = FoneroBase.Memo.fromXDRObject(memo);
    expect(baseMemo.type).to.be.equal(FoneroBase.MemoReturn);
    expect(Buffer.isBuffer(baseMemo.value)).to.be.true;
    expect(baseMemo.value.length).to.equal(32);
    expect(baseMemo.value.toString('hex')).to.be.equal(buffer.toString('hex'));
  });

  var methods = [FoneroBase.Memo.hash, FoneroBase.Memo.return];
