...
[Transactions](https://fonero.org/developers/learn/concepts/transactions.html) are the commands that modify the state of the ledger.
They include sending payments, creating offers, making account configuration changes, etc.

Every transaction has a source [account](https://fonero.org/developers/learn/concepts/accounts.html). This is the account
that pays the [fee](https://fonero.org/developers/learn/concepts/fees.html) and uses up a sequence number for the transaction.

Transactions are made up of one or more [operations](https://fonero.org/developers/learn/concepts/operations.html). Each operation also has a source account, which defaults to the transaction's source account.
...
The transaction will use the current sequence number of the given [Account](https://github.com/fonero-project/js-fonero-base/blob/master/src/account.js) object as its sequence number and increments
...
See [operation.js](https://github.com/fonero-project/js-fonero-base/blob/master/src/operation.js) for a list of possible operations you can add.
...
This will return a fully constructed [Transaction](https://github.com/fonero-project/js-fonero-base/blob/master/src/transaction.js).
The returned transaction will contain the sequence number of the source account. This transaction is unsigned. You must sign it before it will be accepted by the Fonero network.


```js
FoneroSdk.Network.useTestNetwork();
// FoneroBase.Network.usePublicNetwork(); if this transaction is for the public network
// Create an Account object from an address and sequence number.
var account=new FoneroBase.Account("GD6WU64OEP5C4LRBH6NK3MHYIA2ADN6K6II6EXPNVUR3ERBXT4AN4ACD","2319149195853854");
...
var transaction = new FoneroBase.TransactionBuilder(account)
        ...
        .addOperation(FoneroBase.Operation.payment({
                ...
                amount: "100.50"  // 100.50 FNO
            }))
        // add a set options operation to the transaction
        .addOperation(FoneroBase.Operation.setOptions({
                signer: {
...
by passing a [memo](https://github.com/fonero-project/js-fonero-base/blob/master/src/memo.js) object when you construct the TransactionBuilder.
...
var transaction = new FoneroBase.TransactionBuilder(account, {memo:memo})
        .addOperation(FoneroBase.Operation.payment({
                destination: "GASOCNHNNLYFNMDJYQ3XFMI7BYHIOCFW3GJEOWRPEGK2TDPGTG2E5EDW",
                asset: FoneroBase.Asset.native(),
                amount: "2000"
...
more, please consult the [multi-sig documentation](https://fonero.org/developers/learn/concepts/multi-sig.html).

You add signatures to a transaction with the `Transaction.sign()` function. You can chain multiple `sign()` calls together.

## `Keypair` class

`Keypair` object represents key pair used to sign transactions in Fonero network. `Keypair` object can contain both a public and private key, or only a public key.
